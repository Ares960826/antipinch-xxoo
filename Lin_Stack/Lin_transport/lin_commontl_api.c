 ...
 * @{ */
void ld_init(void) {
    /* 初始化发送队列 设置了队列的头、尾、状态和当前大小*/
    lin_tl_tx_queue.queue_header = 0;
    lin_tl_tx_queue.queue_tail = 0;   // 设置发送队列的尾部索引为0
    lin_tl_tx_queue.queue_status = LD_QUEUE_EMPTY; // 设置发送队列的状态为"空"
    lin_tl_tx_queue.queue_current_size = 0; // 设置发送队列的当前大小为0

    /* 初始化接收队列 设置了队列的头、尾、状态和当前大小*/
    lin_tl_rx_queue.queue_header = 0;
    lin_tl_rx_queue.queue_tail = 0;
    lin_tl_rx_queue.queue_status = LD_NO_DATA; // 设置接收队列的状态为"无数据"
    lin_tl_rx_queue.queue_current_size = 0;

    /* 初始化接收消息 接收消息的状态、索引和大小*/
    tl_rx_msg_status = LD_COMPLETED; // 设置接收消息的状态为"已完成"
    tl_rx_msg_index = 0; // 设置接收消息的当前索引
    tl_rx_msg_size = 0;  // 设置接收消息的大小
    tl_receive_msg_status = LD_NO_MSG; // 设置接收消息的状态为"无消息"

    /* 初始化发送消息 */
    tl_tx_msg_status = LD_COMPLETED; // 设置发送消息的状态为"已完成"
    tl_tx_msg_index = 0;
    tl_tx_msg_size = 0;

    tl_last_cfg_result = LD_SUCCESS; // 设置最后的配置结果为"成功"
    tl_last_RSID = 0;    // 设置最后的RSID为0
    tl_ld_error_code = 0; // 设置错误代码为0

    tl_frame_counter = 0; // 设置帧计数器为0
    tl_no_of_pdu = 0;     // 设置PDU数量为0
    tl_slaveresp_cnt = 0; // 设置从设备响应计数器为0

    tl_check_timeout_type = LD_NO_CHECK_TIMEOUT; // 设置检查超时类型为"不检查超时"
    tl_check_timeout = N_MAX_TIMEOUT_CNT; // 设置检查超时计数器为最大超时计数

    tl_diag_state = LD_DIAG_IDLE; 		// 设置诊断状态为"空闲"
    tl_service_status = LD_SERVICE_IDLE; // 设置服务状态为"空闲"
}
/** @} */
/* RAW APIs */
/** @addtogroup raw_api_group
