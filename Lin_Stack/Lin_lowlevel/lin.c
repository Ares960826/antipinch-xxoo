...
{
  #if (_LIN_SCI_ == 1) || (_LIN_XGATE_ == 1)
	// 使用SCI接口或XGATE接口进行LIN通信
  #if (_MCU_ == _S12X_)
  /* if use PIT for time service */
	// 如果MCU是S12X系列，则调用lin_lld_timer_S12X_init()函数来初始化S12X的LIN定时器
  lin_lld_timer_S12X_init();
  // 如果MCU是S12系列，则调用lin_lld_timer_S12_init()函数来初始化S12的LIN定时器
  ...
  // 如果MCU是S08系列，则调用lin_lld_timer_S08_init()函数来初始化S08的LIN定时器
  #endif /* End (_MCU_ == _S08_) */

  #endif /*(_LIN_SCI_ == 1) || (_LIN_XGATE_ == 1) */

  #if (_LIN_SLIC_ == 1)// 使用SLIC接口进行LIN通信
  ...
  #if (_MCU_ == _K_)
  lin_lld_timer_K_init();	// 初始化LIN的定时器
  #endif  /* End (_MCU_ == _S12_) */
